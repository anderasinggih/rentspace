<div>
    ...
    <div>
                                ...
                                <tr>
                                    ...
                                    <td class="relative whitespace-nowrap py-2 sm:py-4 pl-2 pr-2 sm:pr-6 text-right">
                                        ...
                                        <div class="flex items-center gap-2 mt-1">
                                            <button wire:click="openInspect({{ $trx->id }})" class="text-[10px] sm:text-xs text-sky-600 hover:underline">Inspect</button>
        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div class="relative bg-background rounded-xl shadow-2xl w-full max-w-xl border border-border overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-200">
                <!-- Header -->
                <div class="p-6 border-b border-border flex justify-between items-start">
                    <div>
                        ...
                        <p class="text-xs text-muted-foreground mt-0.5">INV-{{ str_pad($inspectTrx->id, 5, '0', STR_PAD_LEFT) }} • {{ $inspectTrx->created_at->format('d M Y') }}</p>
                    </div>
                ...
                <div class="p-6 overflow-y-auto space-y-8 scrollbar-hide">
                    ...
                    <div class="flex items-center justify-between p-4 bg-muted/30 rounded-lg border border-border">
                        <div class="space-y-1">
                            <p class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Status Transaksi</p>
                            <div class="flex items-center gap-2">
                                @if($inspectTrx->status === 'pending') <span class="h-2 w-2 rounded-full bg-amber-500 animate-pulse"></span> <span class="text-sm font-semibold text-amber-600 dark:text-amber-400 uppercase">Menunggu Pembayaran</span>
                                @elseif($inspectTrx->status === 'paid') <span class="h-2 w-2 rounded-full bg-blue-500"></span> <span class="text-sm font-semibold text-blue-600 dark:text-blue-400 uppercase">Dibayar (Aktif)</span>
                                @elseif($inspectTrx->status === 'completed') <span class="h-2 w-2 rounded-full bg-green-500"></span> <span class="text-sm font-semibold text-green-600 dark:text-green-400 uppercase">Selesai</span>
                        ...
                        <div class="text-right space-y-1 border-l border-border pl-4">
                            <p class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Pengembalian Aktual</p>
                            <p class="text-sm font-semibold {{ $inspectTrx->completed_at ? 'text-foreground' : 'text-muted-foreground italic' }}">
                                {{ $inspectTrx->completed_at ? $inspectTrx->completed_at->format('d/m/y H:i') : 'Belum Selesai' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-x-8 gap-y-6">
                        <!-- Customer -->
                        <div class="space-y-4">
                            <h4 class="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/70">Informasi Penyewa</h4>
                            <div class="space-y-4">
                                <div class="group">
                                    <p class="text-[10px] font-bold text-muted-foreground uppercase mb-0.5">Nama Lengkap</p>
                                    <p class="text-sm font-semibold text-foreground leading-tight">{{ $inspectTrx->nama }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-muted-foreground uppercase mb-0.5">Identitas (NIK)</p>
                                    <p class="text-sm font-medium text-foreground tracking-wider">{{ $inspectTrx->nik }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-muted-foreground uppercase mb-0.5">Kontap WhatsApp</p>
                                    <p class="text-sm font-bold text-primary">{{ $inspectTrx->no_wa }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-muted-foreground uppercase mb-0.5">Alamat Domisili</p>
                        ...
                        <!-- Rental -->
                        <div class="space-y-4">
                            <h4 class="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/70">Detail Unit & Waktu</h4>
                            <div class="space-y-4">
                                <div>
                                    <p class="text-[10px] font-bold text-muted-foreground uppercase mb-0.5">Unit Yang Disewa</p>
                                    <p class="text-sm font-bold text-foreground">{{ $inspectTrx->unit->seri ?? 'N/A' }}</p>
                                    <p class="text-[11px] text-muted-foreground">IMEI/SN: {{ $inspectTrx->unit->warna ?? '-' }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-muted-foreground uppercase mb-0.5">Jadwal Mulai</p>
                                    <p class="text-sm font-semibold text-foreground">{{ $inspectTrx->waktu_mulai->format('d M Y, H:i') }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-muted-foreground uppercase mb-0.5">Jadwal Selesai</p>
                                    <p class="text-sm font-semibold text-foreground">{{ $inspectTrx->waktu_selesai->format('d M Y, H:i') }}</p>
                                </div>
                                ...
                                <div class="p-2 bg-primary/5 rounded border border-primary/20">
                                    ...
                                    <p class="text-[11px] font-bold text-primary">{{ $inspectTrx->applied_promo_name }}</p>
                                    @if($inspectTrx->hari_bonus > 0 || $inspectTrx->jam_bonus > 0)
                                    <p class="text-[9px] text-green-600 font-bold">🎁 Bonus +{{ $inspectTrx->hari_bonus > 0 ? $inspectTrx->hari_bonus.'H' : '' }}{{ $inspectTrx->jam_bonus > 0 ? $inspectTrx->jam_bonus.'J' : '' }}</p>
                                    @endif
                                </div>
                                @endif
                            </div>
                    ...
                    <div class="space-y-4 pt-4 border-t border-border">
                        <h4 class="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/70">Ikhtisar Pembayaran</h4>
                        <div class="rounded-lg border border-border overflow-hidden">
                            <table class="w-full text-sm">
                                <tbody class="divide-y divide-border">
                                    <tr class="bg-muted/10">
                                        <td class="py-2.5 px-4 text-muted-foreground">Harga Dasar Sewa</td>
                                        <td class="py-2.5 px-4 text-right font-medium">Rp {{ number_format($inspectTrx->subtotal_harga, 0, ',', '.') }}</td>
                                    </tr>
                                    @if($inspectTrx->potongan_diskon > 0)
                                    <tr>
                                        <td class="py-2.5 px-4 text-red-500">Potongan Diskon / Promo</td>
                                        <td class="py-2.5 px-4 text-right font-medium text-red-500">- Rp {{ number_format($inspectTrx->potongan_diskon, 0, ',', '.') }}</td>
                                    </tr>
                                    @endif
                                    <tr>
                                        <td class="py-2.5 px-4 text-muted-foreground">Kode Unik / Service Fee</td>
                                        <td class="py-2.5 px-4 text-right font-medium">+ Rp {{ $inspectTrx->kode_unik_pembayaran }}</td>
                                    </tr>
                                    @if($inspectTrx->denda > 0)
                                    <tr class="bg-amber-500/5">
                                        <td class="py-2.5 px-4 text-amber-600">Denda Keterlambatan</td>
                                        <td class="py-2.5 px-4 text-right font-bold text-amber-600">+ Rp {{ number_format($inspectTrx->denda, 0, ',', '.') }}</td>
                                    </tr>
                                    @endif
                                    ...
                                    <tr class="bg-red-500/5">
                                        <td class="py-2.5 px-4 text-red-600">
                                            ...
                                            <p class="text-[10px] italic font-normal text-muted-foreground mt-0.5">Ket: {{ $inspectTrx->catatan_kerusakan }}</p>
                                            @endif
                                        </td>
                                        <td class="py-2.5 px-4 text-right font-bold text-red-600">+ Rp {{ number_format($inspectTrx->denda_kerusakan, 0, ',', '.') }}</td>
                                    </tr>
                                    @endif
                                    <tr class="bg-primary/5">
                                        <td class="py-4 px-4 font-bold text-foreground">TOTAL AKHIR</td>
                                        <td class="py-4 px-4 text-right text-lg font-black text-primary">Rp {{ number_format($inspectTrx->grand_total, 0, ',', '.') }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                ...
                <div class="p-6 border-t border-border flex justify-end bg-muted/20">
                    <button wire:click="closeInspect" class="inline-flex items-center justify-center rounded-md text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground shadow hover:bg-primary/90 h-10 px-8">
                        Selesai Meninjau
                    </button>
                </div>
            </div>
