<div class="fixed inset-0 w-screen h-[100dvh] z-[0] bg-background overflow-hidden" 
     x-data="{ 
        ...
        handleTouchStart(e) {
            ...
            this.startH = this.currentH;
        },
        handleTouchMove(e) {
            if (!this.isDragging) return;
            const deltaY = this.startY - e.touches[0].clientY;
            this.currentH = Math.max(this.minH, Math.min(this.maxH, this.startH + deltaY));
        },
        handleTouchEnd() {
            if (!this.isDragging) return;
            this.isDragging = false;
            // Snap logic: if pulled up more than 20% of the way, expand
            if (this.currentH > (this.minH + (this.maxH - this.minH) * 0.25)) {
                this.isExpanded = true;
                this.currentH = this.maxH;
            } else {
        ...
        toggleExpand() {
            ...
            this.currentH = this.isExpanded ? this.maxH : this.minH;
        }
     }">
    {{-- Full Background Map --}}
    ...
    <div
        ...
        <div
            class="flex items-center gap-1 bg-background/60 backdrop-blur-md border border-white/10 p-1 rounded-xl shadow-xl pointer-events-auto">
            <a href="{{ route('admin.monitoring') }}"
                class="px-4 py-1.5 text-[11px] font-semibold hover:bg-white/10 rounded-lg transition-all opacity-60">Monitor</a>
    ...
    <div class="absolute z-[1001]
        ...
        <div
            class="bg-background/70 backdrop-blur-md border border-white/10 rounded-2xl flex flex-col h-full shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden">
            {{-- Unified Drag Handle/Header (Mobile Only) --}}
            <div @click="toggleExpand()"
                @touchstart="handleTouchStart($event)"
                @touchmove.prevent="handleTouchMove($event)"
                @touchend="handleTouchEnd($event)"
                class="h-8 flex lg:hidden items-center justify-center shrink-0 cursor-pointer group hover:bg-white/5 transition-all">
                <div class="w-10 h-1 rounded-full bg-white/20 group-hover:bg-white/40 transition-colors"></div>
            </div>

            <div @click="window.innerWidth < 1024 ? toggleExpand() : null"
                @touchstart="handleTouchStart($event)"
                @touchmove.prevent="handleTouchMove($event)"
                @touchend="handleTouchEnd($event)"
                class="px-8 pt-3 lg:pt-6 pb-3 border-b border-white/5 flex items-center justify-between shrink-0 lg:cursor-default cursor-pointer hover:bg-white/5 lg:hover:bg-transparent transition-all">
                <div>
                    ...
                    <div class="w-full flex items-center px-3 py-2.5 rounded-xl border border-transparent hover:bg-white/5 transition-all group relative cursor-pointer"
                        ...
                        <div class="flex-1 min-w-0 flex items-center justify-between gap-3">
                            <div class="flex-1 min-w-0">
                                <h4
                                    class="text-[14px] font-bold truncate leading-tight  {{ $device['is_overdue'] ? 'text-red-400' : 'text-white' }}">
                                    {{ $device['seri'] }}</h4>
                                <p class="text-[11px] font-medium text-muted-foreground truncate opacity-60 mt-0.5">
                                    {{ $device['nama_peminjam'] }}</p>
                            </div>
                            <div class="text-right shrink-0 flex items-center gap-3">
                                <div class="text-right">
                                    <p
                                        class="text-[12px] font-bold leading-none {{ $device['is_overdue'] ? 'text-red-400' : 'text-emerald-400' }}">
                                        {{ $device['time_left'] }}
                                    </p>
                                    <p class="text-[9px] font-medium text-white/20 mt-1 leading-none">
                                        {{ str_replace('ago', '', $device['last_seen']) }}</p>
                                </div>
                                <a href="{{ route('admin.transactions', ['search' => $device['booking_code']]) }}"
                                    @click.stop
    ...
    <style>
        ...
        .leaflet-tile {
            /* Night Hybrid Filter: Redup kembali */
            filter: brightness(0.70) contrast(1) saturate(1.2);
        }
