                        ...
                        <form wire:submit="save" class="mt-6 space-y-4">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 @if($isIphone) sm:grid-cols-2 @endif gap-4">
                                    <div>
                                        <label
                                            class="text-sm font-medium leading-none">{{ $isIphone ? 'Seri iPhone' : 'Nama Barang' }}</label>
                                        <input type="text" wire:model="seri"
                                            class="mt-1 flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                                            placeholder="{{ $isIphone ? 'iPhone 15 Pro Max' : 'Tripod Takara / Powerbank' }}">
                                ...
                                @if($isIphone)
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="text" wire:model="memori"
                                                class="mt-1 flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                                                placeholder="256 GB">
                                ...
                                @if($selectedCat && $selectedCat->custom_fields && count($selectedCat->custom_fields) > 0)
                                    <div class="pt-4 border-t border-border mt-4">
                                        <label
                                            class="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-3 block">Spesifikasi
                                            Khusus {{ $selectedCat->name }}</label>
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            @foreach($selectedCat->custom_fields as $fieldName)
                                                <div>
                                                    <label class="text-sm font-medium leading-none">{{ $fieldName }}</label>
                                                    <input type="text" wire:model="specs.{{ $fieldName }}"
                                                        class="mt-1 flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                                                        placeholder="Masukkan {{ strtolower($fieldName) }}">
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="number" wire:model="harga_per_jam"
                                            class="mt-1 flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
                                        @error('harga_per_jam') <span class="text-xs text-red-500">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div>
                                        ...
                                        <input type="number" wire:model="harga_per_hari"
                                            class="mt-1 flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
                                        @error('harga_per_hari') <span class="text-xs text-red-500">{{ $message }}</span>
                                        @enderror
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center space-x-2 mt-2">
                                    ...
                                    <label for="is_active" class="text-sm font-medium leading-none cursor-pointer">Unit
