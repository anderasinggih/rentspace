    ...
    <nav class="flex items-center justify-between w-full h-14 bg-background border border-border shadow-md rounded-full px-4 transition-all">
        ...
        <div class="flex items-center">
            ...
            <div class="hidden md:flex items-center gap-6">
                <a href="/" wire:navigate class="text-sm font-medium transition-colors {{ request()->is('/') ? 'text-foreground font-semibold' : 'text-muted-foreground hover:text-foreground' }}">
                    Beranda
                </a>
        ...
        <div class="flex items-center gap-1 sm:gap-3">
            <!-- Dark Mode Toggle -->
            <button @click="toggleTheme()" class="flex p-2 items-center justify-center rounded-full hover:bg-muted text-muted-foreground transition-colors focus:outline-none">
            ...
            <!-- CTA Button -->
            <a href="{{ route('public.booking') }}" wire:navigate class="hidden sm:inline-flex items-center justify-center rounded-full bg-foreground text-background text-xs font-semibold px-5 py-2 hover:bg-foreground/90 transition-all shadow-sm shrink-0">
                Sewa Sekarang
            </a>

            <!-- Mobile Hamburger -->
            <button @click="publicMenuOpen = !publicMenuOpen" class="md:hidden p-2 rounded-full hover:bg-muted text-foreground transition-colors focus:outline-none">
                <svg x-show="!publicMenuOpen" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="4" x2="20" y1="12" y2="12"/><line x1="4" x2="20" y1="6" y2="6"/><line x1="4" x2="20" y1="18" y2="18"/></svg>
                <svg x-show="publicMenuOpen" x-cloak style="display: none;" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
            </button>
        </div>
    </nav>

    <!-- Mobile Dropdown -->
    <div x-show="publicMenuOpen" 
         x-transition
