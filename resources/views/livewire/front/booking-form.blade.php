                                ...
                                <div class="flex items-center gap-2 w-full sm:w-auto">
                                    @if(!$member_checked)
                                        <div class="relative flex-1 sm:w-48">
                                            <input type="text" wire:model.defer="nik" placeholder="Masukkan NIK..." maxlength="16"
                                                class="w-full pl-3 pr-3 py-2.5 text-xs border border-border rounded-xl bg-background focus:ring-1 focus:ring-primary outline-none transition-all shadow-sm font-bold">
                                            @error('nik') <p class="absolute -bottom-5 left-0 text-[10px] text-red-500 font-bold whitespace-nowrap">{{ $message }}</p> @enderror
                                        </div>
                                        <button type="button" wire:click="checkMember" wire:loading.attr="disabled"
                                            class="px-6 py-2.5 bg-primary text-primary-foreground text-xs font-bold rounded-xl shadow-lg shadow-primary/20 hover:shadow-primary/40 hover:-translate-y-0.5 active:translate-y-0 transition-all flex items-center gap-2">
                                    ...
                                    @else
                                        <button type="button" wire:click="$set('member_checked', false); $set('nik', ''); $set('nikFoundMessage', null); $set('isNikVerified', false); calculatePrice();"
                                            class="w-full sm:w-auto px-4 py-2 text-[10px] font-bold text-muted-foreground hover:text-red-500 transition-colors">
                                            Ganti Akun
                                        </button>
                                    @endif
                                </div>
