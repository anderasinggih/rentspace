<div class="py-4 sm:py-12 px-3 sm:px-6 lg:px-8 bg-background min-h-[calc(100vh-4rem)]">
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-background rounded-2xl shadow-sm border border-border p-4 sm:p-8 text-center">
            <h1 class="text-xl sm:text-3xl font-extrabold tracking-tight text-foreground mb-1 sm:mb-4">Selesaikan Pembayaran</h1>
            <p class="text-xs sm:text-sm text-muted-foreground mb-3 sm:mb-8">Tagihan untuk unit <strong>{{ $rental->unit->seri }}</strong>. Mohon transfer tepat hingga tiga digit terakhir agar pembayaran dapat dikenali sistem.</p>

            <div class="flex flex-col items-center justify-center p-4 sm:p-8 bg-muted/30 border border-border rounded-xl gap-3 sm:gap-0">
                ...
                @endphp
                @if($count > 0)
                <div class="inline-flex rounded-md shadow-sm sm:mb-6 mx-2 sm:mx-0 flex-wrap justify-center" role="group">
                    ...
                    @endphp
                    <button
                        wire:click="$set('metode_pembayaran', '{{ $method }}')"
                ...
                <div class="flex items-end justify-center tracking-tight mb-2 sm:mb-6">
                    ...
                    <span class="text-3xl sm:text-5xl font-extrabold text-foreground">{{ number_format($basePrice, 0, ',', '.') }}</span>
                ...
                <div class="w-[220px] h-[220px] sm:w-64 sm:h-64 bg-background border-2 border-dashed border-border rounded-xl overflow-hidden shadow-sm relative">
                    ...
                    <div class="absolute inset-0 flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="text-muted-foreground/30">
                ...
                <div class="mt-2 text-xs text-muted-foreground max-w-xs">
                    Transfer sesuai tiga digit unik (<strong class="text-primary">{{ $lastThree }}</strong>) untuk verifikasi otomatis.
                </div>
                @elseif($metode_pembayaran === 'transfer')
                <!-- Transfer Bank Box -->
                <div class="w-full max-w-xs p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl flex flex-col items-center shadow-sm text-blue-700 dark:text-blue-400">
                    <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mb-2"><rect width="20" height="14" x="2" y="5" rx="2"/><line x1="2" x2="22" y1="10" y2="10"/></svg>
                    <span class="text-sm font-bold text-center">Transfer ke Rekening</span>
                    @php $adminWa = \App\Models\Setting::getVal('admin_wa', ''); @endphp
                    <p class="text-xs text-center mt-1 opacity-80">Konfirmasi via WhatsApp{{ $adminWa ? ' ke ' . $adminWa : '' }} setelah transfer.</p>
                </div>
                @else
                <!-- Cash Box -->
                <div class="w-full max-w-xs p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl flex flex-col items-center shadow-sm text-amber-700 dark:text-amber-400">
                    <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mb-2"><rect width="20" height="12" x="2" y="6" rx="2"/><circle cx="12" cy="12" r="2"/><path d="M6 12h.01M18 12h.01"/></svg>
            ...
            <button
                ...
                <svg wire:loading wire:target="finish"
